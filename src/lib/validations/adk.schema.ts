...
export type FunctionResponseResultContent = z.infer<
  typeof FunctionResponseResultContentSchema
>;
...
export type FunctionResponseResult = z.infer<
  typeof FunctionResponseResultSchema
>;
...
// Support two variations of the create-session response we observe in the wild:
// - older style: { message, data: { id, userId, appName, ... } }
// - newer/alternate style: { message, data: { data: { sessionId, userId, appName, ... } } }
const CreateSessionDataV1 = z.object({
  userId: z.string(),
  ...
  lastUpdateTime: z.union([z.number(), z.string()]).optional().nullable(),
});

const CreateSessionDataV2 = z.object({
  // The API sometimes returns the session info with `sessionId` at the top level
  sessionId: z.string(),
  userId: z.string(),
  appName: z.string(),
...
export const CreateSessionResponseSchema = z.object({
  message: z.string(),
  data: z.union([CreateSessionDataV1, CreateSessionDataV2]),
});
...
const SessionSchema = z.object({
  ...
  userId: z.string(),
  session_name: z.string().optional().nullable(),
  lastUpdateTime: z.union([z.number(), z.string()]).optional().nullable(), // Unix timestamp, typically a number
  appName: z.string(),
});
