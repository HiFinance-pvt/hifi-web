...
export default function HiFiDashboard() {
  ...
  // Add refs to prevent multiple session creation attempts
  ...
  // Check if session exists, if not create a new one - with guards to prevent multiple executions
  useEffect(() => {
    if (
  ...
  }, [sessionId, sessions, router, setCurrentSession, currentSession?.id]); // Removed createSession from dependencies
  ...
  useEffect(() => {
    if (adkResponse && pendingUserMsg) {
      ...
      let assistantText = "";
      if (
        adkResponse.author !== "user" &&
        adkResponse.content &&
        adkResponse.content.parts &&
        adkResponse.content.parts.length > 0
      ) {
        assistantText = adkResponse.content.parts
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                {displayConversations.map((conversation, index) => (
                  <div key={conversation.id} className="space-y-4">
                    ...
                    {conversation.id.includes("pending-user") &&
                      !conversation.response &&
                      isAdkPending && (
                        <div className="flex justify-start">
                          <div className="bg-gray-700 text-white rounded-lg px-4 py-2 max-w-md">
