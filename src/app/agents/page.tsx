...
import { ArrowRight, Bot, CheckCircle, Brain, Zap, Target, Globe, Bell, Power, Wifi, WifiOff, X } from "lucide-react";
...
const AgentCard: React.FC<{ agent: typeof AGENTS[0] }> = ({ agent }) => {
  ...
  return (
    <Link href={agent.href} className="block w-full max-w-full xl:min-w-[320px] xl:w-[320px] h-[350px] sm:h-[380px] xl:h-[400px] agents-card-unique">
      <div
        ref={cardRef}
      ...
      >
        ...
        />
         ...
         <div className="relative z-10 p-4 sm:p-6">
           ...
           <h3 className="text-base sm:text-lg font-semibold text-white mb-2">{agent.name}</h3>
         ...
         <div className="relative z-10 mt-auto p-4 sm:p-6 pt-0">
           ...
           <div className="flex items-center justify-end mt-3 sm:mt-4">
             <ArrowRight 
               className={`w-3 h-3 sm:w-4 sm:h-4 text-gray-500 transition-all duration-300 ${isHovered ? 'translate-x-1' : ''}`}
...
const ConnectionPopup: React.FC<{
  ...
  const [animationStage, setAnimationStage] = useState<'initial' | 'transition' | 'final'>('initial');
  ...
  const getWifiIconProps = () => {
    if (isConnected) {
      ...
      switch (animationStage) {
        case 'initial':
          return { color: 'text-gray-400', bgColor: 'bg-gray-500/20', glowing: false };
        case 'transition':
          return { color: 'text-emerald-400', bgColor: 'bg-emerald-500/20', glowing: false };
        case 'final':
          return { color: 'text-emerald-400', bgColor: 'bg-emerald-500/20', glowing: true };
        default:
          return { color: 'text-gray-400', bgColor: 'bg-gray-500/20', glowing: false };
      }
    } else {
          ...
          <div className="mb-4 sm:mb-6 relative">
            ...
            >
              <Wifi 
                ...
                style={glowing ? {
                  filter: 'drop-shadow(0 0 8px rgb(52 211 153 / 0.6)) drop-shadow(0 0 16px rgb(52 211 153 / 0.4))'
                } : {}}
              />
    ...
    <>
            ...
            {languages.map((lang) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-800/50 transition-colors ${
                  language === lang.code ? 'text-emerald-400' : 'text-gray-300'
      ...
      >
        ...
        >
          ...
          <div className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 w-2 h-2 rounded-full border border-gray-900 ${
          ...
          >
            ...
            <div className="px-4 py-3 bg-gradient-to-r from-gray-800/50 to-gray-700/50 border-b border-gray-600/30">
              <div className="flex items-center space-x-3">
                <div className={`w-3 h-3 rounded-full ${fiConnected ? 'bg-emerald-400' : 'bg-red-400'} animate-pulse shadow-lg`} />
...
export default function AgentsHubPage() {
  ...
  useEffect(() => {
    ...
    if (container) {
      ...
      return (
      <div ref={containerRef} className="flex flex-col lg:flex-row h-screen w-screen bg-[#111827] overflow-hidden agents-page-container">
      {/* Header Controls */}
      <HeaderControls />
      ...
      {/* Main Content Area */}
      <div className="flex-1 relative overflow-hidden z-10 min-h-0 min-w-0" style={{ pointerEvents: 'auto' }}>
          ...
          <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 w-full">
            ...
            <div className="mb-6 lg:mb-8">
              <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 mb-4">
                ...
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white">Agents Hub</h1>
              </div>
              <p className="text-base sm:text-lg lg:text-xl text-gray-300 max-w-2xl">
                Discover our specialized AI agents designed to help you with various aspects of financial planning and management.
              </p>
            </div>
            ...
            <div className="mt-8 lg:mt-16 p-4 sm:p-6 lg:p-8 rounded-xl lg:rounded-2xl bg-gradient-to-r from-gray-900/40 to-gray-800/40 border border-gray-700/50 max-w-4xl w-full backdrop-blur-sm">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-semibold text-white mb-3 sm:mb-4">Why Choose Our AI Agents?</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                <div className="space-y-2">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <h3 className="text-base sm:text-lg font-medium text-yellow-400">Expert Knowledge</h3>
                  </div>
                  <p className="text-gray-300 text-xs sm:text-sm">Each agent is trained on domain-specific expertise to provide accurate and relevant guidance.</p>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <h3 className="text-base sm:text-lg font-medium text-cyan-400">Real-time Insights</h3>
                  </div>
                  <p className="text-gray-300 text-xs sm:text-sm">Get up-to-date market data and regulatory information for informed decision making.</p>
                </div>
                <div className="space-y-2 sm:col-span-2 lg:col-span-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <h3 className="text-base sm:text-lg font-medium text-red-400">Personalized Advice</h3>
                  </div>
                  <p className="text-gray-300 text-xs sm:text-sm">Receive customized recommendations based on your financial goals and risk profile.</p>
                </div>
              </div>
            </div>
          </div>
